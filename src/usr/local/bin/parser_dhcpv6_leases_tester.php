...
$FreeSense_platform_file = '/etc/platform';

$is_FreeSense = file_exists($FreeSense_platform_file);
if ($is_FreeSense) {
	if ( ! preg_match('/^FreeSense/i', file_get_contents($FreeSense_platform_file))) {
		$is_FreeSense = false;
	}
}
...
if ($is_FreeSense) {
	...
	/* Handle situation where we use source files on a non-running FreeSense system.
