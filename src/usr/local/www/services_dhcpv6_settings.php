...
##|+PRIV
##|*IDENT=page-services-dhcpv6server-settings
##|*NAME=Services: DHCPv6 Server: Settings
##|*DESCR=Allow access to the 'Services: DHCPv6 Server: Settings' page.
##|*MATCH=services_dhcpv6_settings.php*
##|-PRIV
...
$pgtitle = array(gettext('Services'), gettext('DHCPv6 Server'), gettext('Settings'));
...
if (is_subsystem_dirty('dhcpd6')) {
	print_apply_box(gettext('The DHCPv6 Server configuration has changed.') . '<br />' . gettext('The changes must be applied for them to take effect.'));
}
...
if ($tabscounter === 0) {
	print_info_box(gettext('DHCPv6 Server can only be enabled on interfaces configured with a static IPv6 address. This system has none.'), 'danger', false);
}
...
))->setHelp(gettext('Default DNS registration policy for DHCPv6 server.%1$s' .
	'When checked, DHCPv6 leases will automatically be registered with the DNS Resolver.'), '<br/>');
...
))->setHelp(gettext('Default early DNS registration policy for DHCPv6 server.%1$s' .
			'When checked, DHCPv6 static mappings will automatically be pre-registered with the DNS Resolver.'), '<br/>');
...
$group->add(new Form_Input(
	...
	['min' => 1, 'max' => 65535, 'placeholder' => kea_defaults('listenport') + 1]
))->addClass('autotrim')
  ->setWidth(2)
